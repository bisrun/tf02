...
def run():
    ...
    try :
        ...
        node =  pd.read_csv(file_in_node, sep='\|', usecols=['NODE_ID','NODE_CATE','ADJ_NODE_ID','X','Y'],
                                )
        ...
        link = pd.read_csv(file_in_link, sep='\|', usecols=['LINK_ID', 'FROM_NODE_ID', 'TO_NODE_ID', 'ROUTE_ROAD_CATE'])
        link_cmid = pd.merge(link, geom_df, how='inner', left_on='FROM_NODE_ID', right_on='NODE_ID')

        dust = pd.read_csv(dust_fp, sep='\t', engine='python', usecols=['REG_ID', 'UV', 'PM10', 'CAI'], dtype={'REG_ID':str})
